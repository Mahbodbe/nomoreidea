...
(() => {
  ...
  for(const el of document.querySelectorAll('body *')){
    ...
    const bg=bgOf(el), a=lum(fg), b=lum(bg), ratio=(Math.max(a,b)+.05)/(Math.min(a,b)+.05);
    ...
    async with websockets.connect(ws_url, max_size=40_000_000) as ws:
        ...
        for lang in ("en", "fa"):
            ...
            for width in WIDTHS:
                ...
                # Wait for the actual page DOM and language UI, not just readyState.
                ready = False
                for _ in range(100):
                    q = await send("Runtime.evaluate", {
                        "expression": "({ready:document.readyState, toggle:!!document.getElementById('langToggle')})",
                        "returnByValue": True,
                    })
                ...
                if not ready:
                    raise RuntimeError(f"page/i18n UI not ready at {width}px ({lang})")

                # Make the requested language the one shown by using the same UI control a user uses.
                q = await send("Runtime.evaluate", {
